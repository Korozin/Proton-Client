if __name__ == "__main__":
    print("This is a module that is imported by 'ProtonClient.py'. Don't run it directly.")
    exit()
else:
    import socket, struct, colorama
...
class uGecko:
    ...
    def __init__(self, ip: str, port: int = 7331, debug_mode: bool = False) -> None:
        ...
        if not IP_Verification(ip):
            raise InvalidIpAddressException("The IP address given is not valid!")
            ...
            if not self.__connected:
                try:
                    if self.__debug_mode:
                        print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} Connecting to {self.__ip}:{self.__port}...")
                    self.s.settimeout(timeout)
                    self.s.connect((str(self.__ip), int(self.__port)))
                    self.__connected = True
                    if self.__debug_mode:
                        print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} Connected!")
                except Exception as e:
                    if self.__debug_mode:
                        print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} Connection Failed :(")
                        print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} Raised Error: {colorama.Fore.YELLOW}'ConnectionErrorException'{colorama.Fore.RESET}")
                    raise ConnectionErrorException(f"Unable to connect to {self.__ip}\nRaw Error: {e}") 
            else:
                raise ConnectionIsAlreadyInProgressException(f"A connection to {self.__ip} is already in progress!")
        else:
            raise NoIpEnteredException("No IP address has been specified to connect!")


    def disconnect(self):
        if not self.__connected:
            raise ConnectionIsNotInProgressException("No connection is in progress!")
        self.s.close()
        self.__connected = False
    ...
    def poke8(self, address: int, value: int, skip_verification: bool = False) -> None:
        if not self.__connected:
            raise ConnectionIsNotInProgressException("No connection is in progress!")
        if not Memory.isValidMemoryArea(address, 1, skip_verification):
            raise InvalidMemoryAreaException("The address given is not valid!")

        self.s.send(Commands.POKE_8.value)
        self.s.send(struct.pack(">II", address, value))


    def poke16(self, address: int, value: int, skip_verification: bool = False) -> None:
        if not self.__connected:
            raise ConnectionIsNotInProgressException("No connection is in progress!")
        if not Memory.isValidMemoryArea(address, 4, skip_verification):
            raise InvalidMemoryAreaException("The address given is not valid!")

        self.s.send(Commands.POKE_16.value)
        self.s.send(struct.pack(">II", address, value))
    ...
    def poke32(self, address: int, value: int, skip_verification: bool = False) -> None:
        if not self.__connected:
            raise ConnectionIsNotInProgressException("No connection is in progress!")
        if not Memory.isValidMemoryArea(address, 8, skip_verification):
            raise InvalidMemoryAreaException("The address given is not valid!")

        self.s.send(Commands.POKE_32.value)
        self.s.send(struct.pack(">II", address, value))
    ...
    def read(self, address: int, length: int = 4, skip_verification: bool = False) -> bytearray:
        ...
        if not Memory.isValidMemoryArea(address, length, skip_verification, "read"):
            raise InvalidMemoryAreaException("The address given is not valid!")

        if length == 0:
            raise InvalidLengthException("The length given is not valid!")
    ...
    def __read(self, address: int, length: int) -> bytearray:
        self.s.send(Commands.READ_MEMORY.value)
        self.s.send(struct.pack(">II", address, address + length))
    ...
    def kernelWrite(self, address: int, value: int, skip_verification: bool = True) -> None:
        if not self.__connected:
            raise ConnectionIsNotInProgressException("No connection is in progress!")
        ...
        self.s.send(Commands.KERNEL_WRITE.value)
        self.s.send(struct.pack(">II", address, value))
    ...
    def kernelRead(self, address: int, skip_verification: bool = False) -> int:
        ...
        if not Memory.isValidMemoryArea(address, 4, skip_verification, "read"):
            raise InvalidMemoryAreaException("The address given is not valid!")

        self.s.send(Commands.KERNEL_READ.value)
        self.s.send(struct.pack(">I", address))

        return int.from_bytes(self.s.recv(4), byteorder="big")
    ...
    def getServerStatus(self) -> int:
        ...
        self.s.send(Commands.SERVER_STATUS.value)

        return int.from_bytes(self.s.recv(1), byteorder="big")
    ...
    def isConsolePaused(self) -> bool:
        ...
        self.s.send(Commands.IS_CONSOLE_PAUSED.value)

        return bool(int.from_bytes(self.s.recv(1), byteorder="big"))
    ...
    def pauseConsole(self) -> None:
        ...
        self.s.send(Commands.PAUSE_CONSOLE.value)
    ...
    def resumeConsole(self) -> None:
        ...
        self.s.send(Commands.RESUME_CONSOLE.value)
    ...
    def getServerVersion(self) -> str:
        ...
        self.s.send(Commands.GET_SERVER_VERSION.value)

        return self.s.recv(16).decode("UTF-8").replace("\n", "")
    ...
    def getOsVersion(self) -> int:
        ...
        self.s.send(Commands.GET_OS_VERSION.value)
    ...
    def getVersionHash(self) -> int:
        ...
        self.s.send(Commands.GET_VERSION_HASH.value)
    ...
    def getAccountID(self) -> str:
        ...
        self.s.send(Commands.ACCOUNT_IDENTIFIER.value)

        return hex(int.from_bytes(self.s.recv(4), byteorder="big")).replace("0x", "")
    ...
    def getCodeHandlerAddress(self) -> int:
        ...
        self.s.send(Commands.GET_CODE_HANDLER_ADDRESS.value)
    ...
    def getDataBufferSize(self) -> int:
        ...
        self.s.send(Commands.GET_DATA_BUFFER_SIZE.value)
    ...
    def getSymbol(self, rpl_name: str, symbol_name: str, data = 0) -> int:
        ...
        self.s.send(Commands.GET_SYMBOL.value)
    ...
    def call(self, address, *args, recv: int = 4):
        ...
        if len(arguments) <= 8:
            ...
            self.s.send(Commands.REMOTE_PROCEDURE_CALL.value)
            self.s.send(req)

            return struct.unpack('>Q', self.s.recv(8))[0] >> 32 * (recv == 4)
        else:
    ...
    def search(self, start_address: int, length: int, value: int, skip_verification: bool = False) -> int:
        ...
        if not Memory.isValidMemoryArea(start_address, length, skip_verification, "read"):
            raise InvalidMemoryAreaException("Invalid memory area!")

        self.s.send(Commands.MEMORY_SEARCH.value)
        self.s.send(struct.pack(">III", start_address, value, length))
    ...
    def advancedSearch(self, start_address: int, length: int, value: int, kernel: int, limit: int, aligned: int = 1, skip_verification: bool = False) -> list:
        ...
        self.s.send(Commands.ADVANCED_MEMORY_SEARCH.value)
        ...
        count = int.from_bytes(self.s.recv(4), byteorder="big") / 4
        foundOffsets = list()
        for i in range(int(count)):
            foundOffsets.append(int.from_bytes(self.s.recv(4), byteorder="big"))
        return foundOffsets
    ...
    def getEntryPointAddress(self) -> int:
        ...
        self.s.send(Commands.GET_ENTRY_POINT_ADDRESS.value)
    ...
    def runKernelCopyService(self) -> None:
        ...
        self.s.send(Commands.RUN_KERNEL_COPY_SERVICE.value)
    ...
    def clearAssembly(self) -> None:
        ...
        self.s.send(Commands.CLEAR_ASSEMBLY.value)
    ...
    def executeAssembly(self, assembly: str) -> None:
        ...
        self.s.send(Commands.EXECUTE_ASSEMBLY.value)
        self.s.send(assembly.encode("UTF-8"))
    ...
    def persistAssembly(self, assembly: str) -> None:
        ...
        self.s.send(Commands.PERSIST_ASSEMBLY.value)
        self.s.send(assembly.encode("UTF-8"))
    ...
    def __upload(self, startAddress: int, data: bytes) -> None:
        self.s.send(Commands.UPLOAD_MEMORY.value)
        req = struct.pack(">II",startAddress, startAddress+len(data))
        self.s.send(req)  # first let the server know the length
        self.s.send(data) # then send the data
    ...
    def upload(self, startAddress: int, data: bytes, debug_print:bool = False) -> None:
        if self.__connected:
            ...
            if length > maxLength:
                pos = 0
                if debug_print: print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} length over {colorama.Fore.YELLOW}'{hex(maxLength)}'{colorama.Fore.RESET}\nuploading in blocks!")
            ...
            if length != 0: self.__upload(startAddress, data[pos:pos+length])
                ...
                self.__upload(startAddress, data)
        else:
            raise ConnectionIsNotInProgressException("No connection is in progress!")
